...
public class HexGridManager : MonoBehaviour
{
    ...
    public float tileWidth = 1.0f;  // Exact width of the hex tile
    public float tileHeight = 1.0f; // Exact height of the hex tile
    public int rows = 4;            // Number of rows in the grid
    public int columns = 7;         // Number of columns in the grid

    [Header("Spacing Settings")]
    public float spacing = 0.2f;    // Consistent spacing for both horizontal and vertical gaps
    ...
    private void GenerateHexGrid()
    {
        ...
        float gridOriginX = -gridWidth / 2f + (tileWidth / 2f);
        float gridOriginY = -gridHeight / 2f + (tileHeight / 2f) + verticalShift;
